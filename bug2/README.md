...
[gets transformed](https://ligkern.dev/?font=cmr10&text=difficult) into a single ligature `ﬃ`
...
It then tries to ensure that if the right boundary character is replaced, the replacement is not put in the input because the replacement
  already exists after the word.
Example: if j=n.
